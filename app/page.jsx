    ...
    <div className="p-3 flex flex-col gap-5 font-poppins pb-10 pt-16">
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-5">
        ...
        <div className="flex flex-col gap-2">
          <label htmlFor="question" className="font-medium text-white">
            Question
            <span className="text-dark-500 text-xs pl-3 italic font-normal">
              min 10 characters
            </span>
          </label>

          <textarea
            className={`p-3 border border-dark-900 rounded-md focus:outline-none placeholder:text-dark-700 focus:border-dark-500 resize-none bg-dark-900 text-white ${
              errorQuestion && "border-statusLate"
            }`}
            rows={4}
            id="question"
          ...
          <div
            className={`flex items-center ${
              errorQuestion ? "justify-between" : "justify-end"
            }`}
          >
            {errorQuestion && (
              <p className="text-statusLate text-xs">{errorQuestion}</p>
            )}
        ...
        <div className="flex flex-col gap-2 ">
          ...
          <div className="flex gap-2">
            <div className="flex flex-col gap-2 w-full">
              <input
                className={`w-full p-3 border border-dark-900 rounded-md focus:outline-none placeholder:text-dark-700 focus:border-dark-500 bg-dark-900 text-white ${
                  errorAnswerYes && "border-statusLate"
                }`}
                type="text"
                id="yes"
                placeholder="Yes"
            ...
            <div className="flex flex-col gap-2 w-full">
              <input
                className={`w-full p-3 border border-dark-900 rounded-md focus:outline-none placeholder:text-dark-700 focus:border-dark-500 bg-dark-900 text-white ${
                  errorAnswerNo && "border-statusLate"
                }`}
                type="text"
                id="no"
                placeholder="No"
        ...
        <div className="flex flex-col gap-2 mt-5">
          ...
          <input
            className={`p-3 border border-dark-900 rounded-md focus:outline-none placeholder:text-dark-700 focus:border-dark-500 bg-dark-900 text-white ${
              errorResultTitle && "border-statusLate"
            }`}
            type="text"
            id="title"
            placeholder="Yeayy!"
          ...
          <div
            className={`flex items-center ${
              errorResultTitle ? "justify-between" : "justify-end"
            }`}
          >
            {errorResultTitle && (
              <p className="text-statusLate text-xs">{errorResultTitle}</p>
            )}
            <div className="text-dark-500 text-sm">{resultTitle.length}/10</div>
          </div>

          <textarea
            className={`mt-3 p-3 border border-dark-900 rounded-md focus:outline-none placeholder:text-dark-700 focus:border-dark-500 resize-none bg-dark-900 text-white ${
              errorResultDescription && "border-statusLate"
            }`}
            rows={4}
            id="question"
          ...
          <div
            className={`flex items-center ${
              errorResultDescription ? "justify-between" : "justify-end"
            }`}
          >
            {errorResultDescription && (
              <p className="text-statusLate text-xs">
                {errorResultDescription}
              </p>
            )}
          ...
          <div className="mt-3 bg-dark-700 text-white w-full p-3 min-h-[150px] rounded-md flex flex-col gap-3 relative break-all">
            <div className="text-lg font-bold">{resultTitle}</div>
        ...
        <button
          type="submit"
          className="w-full mt-5 text-white rounded-md bg-dark-900 h-12 font-medium hover:bg-dark-500"
        >
          Generate Link
        </button>
      </form>
      ...
      <div className="w-full p-3 min-h-[10rem] bg-dark-900 rounded-md break-all">
        <p
          ...
          className="text-white text-sm placeholder:text-dark-500"
        >
          {link}
        </p>
      </div>
      ...
      <button
        onClick={handleCopy}
        className={`hidden md:block w-full text-white rounded-md  h-12 font-medium hover:bg-dark-500 ${
          copy ? "bg-dark-500" : "bg-dark-900"
        }`}
        aria-disabled={copy}
      >
